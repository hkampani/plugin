<?php
/*
Plugin Name: large pulgins
Version: 1.0
Plugin URI: https://himanshuk.sgedu.site/real/cars44
Description: its upload large files for pulgins without any restriction or error. 
...
// Globals
define('PLUGINSUPLOAD_VERSION', '1.0');
define('PLUGINSUPLOAD_SLUG', "upload-larger-plugins");
define('PLUGINSUPLOAD_DIR', dirname(realpath(__FILE__)));
define('PLUGINSUPLOAD_URL', plugins_url('', __FILE__));
...
// 		if (version_compare($wp_version, '3.7', '>=')) return;
		...
		$upgrader->strings = array('download_failed' => __('Error when trying to find uploaded file', 'PLUGINSUPLOAD'));
...
// 			if (!is_writable($upload_dir)) return;
	...
	public function admin_enqueue_scripts() {
		...
		wp_enqueue_script('ulp-admin-ui', PLUGINSUPLOAD_URL.'/admin.js', array('jquery', 'plupload-all'), '1');

		wp_localize_script('ulp-admin-ui', 'ulplion', array(
			'notarchive' => __('This file does not appear to be a zip file.', 'PLUGINSUPLOAD'),
			'notarchive2' => '<p>'.__('This file does not appear to be a zip file.', 'PLUGINSUPLOAD').'</p>',
			'uploaderror' => __('Upload error:','PLUGINSUPLOAD'),
			'makesure' => __('(make sure that you were trying to upload a zip file','PLUGINSUPLOAD'),
			'uploaderr' => __('Upload error', 'PLUGINSUPLOAD'),
			'jsonnotunderstood' => __('Error: the server sent us a response (JSON) which we did not understand.', 'PLUGINSUPLOAD'),
			'error' => __('Error:','PLUGINSUPLOAD')
		));
	...
	public function load_translations() {
		...
		load_plugin_textdomain('PLUGINSUPLOAD', false, basename(dirname(__FILE__)).'/languages/');
	}
	...
	public function ulp_plupload_action() {
		...
		check_ajax_referer('PLUGINSUPLOAD-uploader');
...
// 		}
		...
		if (!isset($_POST['chunks']) || (isset($_POST['chunk']) && $_POST['chunk'] == $_POST['chunks']-1)) {
			...
			if (!preg_match('/\.zip$/i', $file, $matches)) {
				@unlink($status['file']);
				echo json_encode(array('e' => sprintf(__('Error: %s', 'PLUGINSUPLOAD'), __('This file does not appear to be a zip file.', 'PLUGINSUPLOAD'))));
				exit;
			}
		}
	...
	public function admin_head() {
		...
		$plupload_init = array(
			...
			'multipart_params' => array(
				'_ajax_nonce' => wp_create_nonce('PLUGINSUPLOAD-uploader'),
...
// 			'silverlight_xap_url' => includes_url('js/plupload/plupload.silverlight.xap'),
	...
	<div class="upload-plugin">
		...
		if (version_compare($wp_version, '3.3', '<')) {
			echo '<em>'.sprintf(__('This feature requires %s version %s or later', 'PLUGINSUPLOAD'), 'WordPress', '3.3').'</em>';
		} else {
			?>
			<div id="plupload-upload-ui" class="drag-drop" style="width: 70%;">
				<div id="drag-drop-area">
					<div class="drag-drop-inside">
					<p class="drag-drop-info"><?php _e('Drop plugin zip here', 'PLUGINSUPLOAD'); ?></p>
					<p><?php _ex('or', 'Uploader: Drop plugin zip here - or - Select File'); ?></p>
					<p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php echo esc_attr(__('Select File', 'PLUGINSUPLOAD')); ?>" class="button" /></p>
					</div>
				</div>
				<div id="filelist">
				</div>
...
*/
	...
	public function action_links($links, $file) {
		if ($file == PLUGINSUPLOAD_SLUG."/".basename(__FILE__)) {
			array_unshift( $links, 
				'<a href="options-general.php?page=upload_larger_plugins">'.__('Settings', 'PLUGINSUPLOAD').'</a>'
			);
		}
		return $links;
	}
